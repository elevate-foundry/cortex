...
def cmd_elect(args) -> int:
    """Run tier election — models grade themselves and each other via gossip consensus."""
    from .tier_election import run_tier_election, DEFAULT_CANDIDATES

    print(f"╔══════════════════════════════════════════════════════════════╗")
    print(f"║  Cortex Tier Election                                        ║")
    print(f"║  Gossip protocol applied to model identity                   ║")
    print(f"║  Models self-assess → cross-verify → reach consensus         ║")
    print(f"╚══════════════════════════════════════════════════════════════╝")
    print()
    ...
    subset = None
    if args.models:
        subset = [m.strip() for m in args.models.split(",")]

    candidates = None
    if args.local_only:
        # Filter to models that could run locally
        candidates = [c for c in DEFAULT_CANDIDATES if c.param_size_b <= 70]

    result = run_tier_election(
        output_dir=args.output_dir,
        subset=subset,
...
def cmd_distill(args) -> int:
    """Run the distillation pipeline — frontier model → CKM training data."""
    from .distill import run_distillation
    from .distill_traces import save_distill_traces

    print(f"╔══════════════════════════════════════════════════════════════╗")
    print(f"║  Cortex Distillation Pipeline                                ║")
    print(f"║  Teacher: frontier model reasoning → CKM training data       ║")
    print(f"║  Count: {args.count} examples per source                            ║")
    print(f"╚══════════════════════════════════════════════════════════════╝")
    print()
    ...
    # Phase 1: Labeled decisions + reasoning traces
    print("[1/3] Generating labeled decisions and reasoning traces...")
    stats = run_distillation(
        count=args.count,
        output_path=str(output_dir),
        include_reasoning_traces=True,
    )
    print(f"  Decisions: {stats['decisions']['total']} examples")
    print(f"  Reasoning: {stats['reasoning'].get('total_traces', 0)} traces")
    print()

    # Phase 2: Deep distillation traces
    print("[2/3] Generating deep distillation traces...")
    trace_path = str(output_dir / "distill_traces.jsonl")
    trace_stats = save_distill_traces(trace_path, count=args.count)
    print(f"  Traces: {trace_stats['total_traces']} generated")
    print(f"  Types: {json.dumps(trace_stats['trace_types'])}")
    print()

    # Phase 3: Merge into unified training file
    print("[3/3] Merging into unified training corpus...")
    merged_path = output_dir / "distilled_corpus.jsonl"
    total_lines = 0
    with open(merged_path, "w") as out:
        for source_file in sorted(output_dir.glob("*.jsonl")):
            if source_file.name == "distilled_corpus.jsonl":
                continue
            ...
            try:
                ...
                continue
    print(f"  Merged: {total_lines} total training examples")
    print(f"  Output: {merged_path}")
    print()

    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print(f"  Total distilled examples: {total_lines}")
    print(f"  Ready for: python -m src train run --data-dir {output_dir}")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
def main(argv=None) -> int:
    ...
    # Distill subcommand — frontier model → CKM training data
    distill_parser = sub.add_parser("distill", help="Run distillation pipeline (frontier → CKM)")
    distill_parser.add_argument("--count", type=int, default=5000,
                               help="Number of examples per source to generate")
    distill_parser.add_argument("--output-dir", default="/tmp/cortex-train/distill",
                               help="Output directory for distilled training data")
    distill_parser.set_defaults(func=cmd_distill)

    # Elect subcommand — tier election via gossip consensus
    elect_parser = sub.add_parser("elect", help="Run tier election (models grade each other via OpenRouter)")
    elect_parser.add_argument("--output-dir", default="/tmp/cortex/tier_election",
                             help="Output directory for election results")
    elect_parser.add_argument("--models", default=None,
                             help="Comma-separated model IDs to include (default: all candidates)")
    elect_parser.add_argument("--local-only", action="store_true",
                             help="Only include models that could run locally (≤70B)")
    elect_parser.set_defaults(func=cmd_elect)
