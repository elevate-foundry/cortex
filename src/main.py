...
def main():
    ...
    # sleep — prepare for eject
    p_sleep = sub.add_parser("sleep", help="Prepare Cortex for drive ejection (graceful shutdown)")
    p_sleep.add_argument("--reason", default="user_requested", help="Reason for sleep")

    # dream — manage the dream agent
    p_dream = sub.add_parser("dream", help="Manage dream mode (background learning while unplugged)")
    dream_sub = p_dream.add_subparsers(dest="dream_command", help="Dream subcommand")
    dream_sub.add_parser("install", help="Install the dream launchd agent")
    dream_sub.add_parser("uninstall", help="Remove the dream launchd agent")
...
def cmd_sleep(args):
    """Graceful shutdown — prepare for drive ejection."""
    from .lifecycle import cmd_sleep as _sleep
    _sleep(reason=args.reason)


def cmd_dream(args):
    """Manage the dream agent (background learning while unplugged)."""
    ...
    subcmd = getattr(args, "dream_command", None)
    if subcmd == "install":
        cmd_dream_install()
